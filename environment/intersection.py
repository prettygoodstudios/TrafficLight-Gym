...
class Intersection:
    __slots__ = ['__lanes', '__geometry', '__yieldZones', '__visualizer', '__phase', '__phaseCount']
    ...
    def getTotalIdleTime(self) -> int:
        ...
        def inYieldZone(vehicle: Vehicle) -> bool:
            return True in (zone.intersects(vehicle.geometry) for zone in self.__yieldZones[LightPhase.AllRed])
        return sum([sum((vehicle.idleTime for vehicle in lane.getVehicles() if inYieldZone(vehicle))) for lane in self.__lanes])
    ...
    def getVehiclesWithinYieldZones(self):
        ...
        for zone in zones:
            vehicles.append([])
            for lane in self.__lanes:
                vehicles[-1].extend(lane.getVehicles(lambda v: v.geometry.intersects(zone) and abs(sum(v.velocity)) < 0.4))
        return vehicles
    ...
    def render(self):
        ...
        if self.__visualizer:
            self.__visualizer.render(self.__lanes, self.__phase, self.__geometry)
